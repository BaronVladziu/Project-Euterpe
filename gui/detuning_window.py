...
class DetuningWindow(PageWindow):
    def __init__(self):
        super().__init__()
        ...
        # Add action button
        self.action_button = QtWidgets.QPushButton("Generate New Detuned Melody", self)
        grid_layout.addWidget(
            self.action_button,
            1, 3,
            alignment=QtCore.Qt.AlignCenter
    ...
    def reset_window(self):
        self.state_label.setText("Press button to generate new example -->")
        self.action_button.setText("Generate New Detuned Melody")
        self.label.if_active = False

    def make_handleButton(self, button):
        def handleButton():
            if button == "settings_button":
                self.goto("detuning_settings_page")
            elif button == "generator_button":
                self.goto("detuning_generator_page")
...
class DetuningGeneratorWindow(PageWindow):
    def __init__(self, parent:DetuningWindow):
        super().__init__()
        self.parent = parent
...
class DetuningSettingsWindow(PageWindow):
    def __init__(self, parent:DetuningWindow):
        super().__init__()
        self.parent = parent
        ...
        self.voice_length_list.setCurrentIndex(3)
        grid_layout.addWidget(
        ...
        self.max_detuning_label = QtWidgets.QLabel()
        self.max_detuning_label.setText("Max Detuning:")
        grid_layout.addWidget(
            self.max_detuning_label,
            7, 0,
            alignment=QtCore.Qt.AlignCenter
        )
        self.max_detuning_list = QtWidgets.QComboBox()
        self.max_detuning_list.addItems([
            "3",
            "5",
            "10",
            "20",
        ...
        self.max_detuning_list.setCurrentIndex(5)
        grid_layout.addWidget(
            self.max_detuning_list,
            7, 1,
            alignment=QtCore.Qt.AlignCenter
        )
        self.max_detuning_list.currentIndexChanged.connect(
            self.max_detuning_changed
        )
        self.max_detuning_changed()
        ...
        self.possible_error_list.setCurrentIndex(4)
        grid_layout.addWidget(
    ...
    def max_detuning_changed(self):
        self.parent.exercise.set_max_detuning(
            float(self.max_detuning_list.currentText())
        )
