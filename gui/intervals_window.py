...
class IntervalsWindow(PageWindow):
    def __init__(self):
        ...
        # Add exercise class
        self.intervals_exercise = IntervalsExercise(
        ...
        def handleButton():
            if button == "settings_button":
                ...
                if self.excercise_state == "start_state":
                    ...
                    self.intervals_exercise.generate_new_example()
                    self.intervals_exercise.play_example()
    ...
    def mouseMoveEvent(self, event):
        if self.excercise_state == "test_state":
            ...
            max_error = (self.intervals_exercise.get_possible_error()/2)
    ...
    def mousePressEvent(self, event):
        if self.excercise_state == "test_state":
            x = event.x()
            if_correct, true_value = self.intervals_exercise.answer_example(
                2*max(x - 10, 0)
            )
            ...
            if if_correct:
                self.state_label.setText(
                    ...
                    + str(2*max(x - 10, 0))\
                    + "±"\
                    + str(int(self.intervals_exercise.get_possible_error()))\
                    + "c, Real value: "\
                    + str(int(true_value))
                )
...
class IntervalsGeneratorWindow(PageWindow):
    ...
    def synthesizer_type_changed(self):
        if self.synthesizer_type_list.currentText() == "Sine":
            self.parent.intervals_exercise.set_synthesizer(
                SineSynthesizer
            )
    ...
    def sampling_frequency_changed(self):
        self.parent.intervals_exercise.set_sampling_frequency(
...
class IntervalsSettingsWindow(PageWindow):
    ...
    def largest_interval_changed(self):
        self.parent.intervals_exercise.set_largest_interval(
            Interval.from_cents(
    ...
    def possible_detune_changed(self):
        self.parent.intervals_exercise.set_possible_detune(
            int(self.possible_detune_list.currentText())
        )

    def possible_error_changed(self):
        self.parent.intervals_exercise.set_possible_error(
